...
# Цена в тексте объявления: "$1,250", "1250$", "1250 usd", "1250 per month", "1250/mo".
# Форматы перечислены отдельными альтернативами, потому что \b ведёт себя
# по-разному рядом с "$" и рядом с буквой: после "$" границы слова нет,
# и общий шаблон с финальным \b молча пропускал вариант "1250$".
PRICE_PATTERN = re.compile(
    r"\$\s?\d[\d,.]*"
    r"|\b\d{3,5}\s?\$"
    r"|\b\d{3,5}\s?(?:usd|per\s+month|/\s?mo(?:nth)?)\b",
    re.I,
)
